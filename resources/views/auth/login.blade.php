<x-layouts.auth>
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">Sign In account</h2>
            <p class="mt-2 text-center text-sm text-gray-600">
                Or
                <a href="{{ route('users.register') }}" class="font-medium text-indigo-600 hover:text-indigo-500"> start your 14-day free trial </a>
            </p>
        </div>
        @error('email')
        ...
        <form class="mt-8 space-y-6" action="{{ route('users.authenticate') }}" method="POST">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Email address">
                </div>
                <div>
